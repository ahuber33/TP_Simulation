...
void Analyse(const char* file_proton, const char* file_He1, const char* file_He2, const char* TreeName, const char* CanvasName, float pinhole, float B, float E, float Lb1, float D, float Le1, float Le2, float Sc_length, float Sc_thickness, float ZnS_thickness, bool Optique)
{
  ...
  float nsigma;
  if (Optique == true) {nsigma = 1.5;}
  if (Optique == false) {nsigma =1.5;}
  //Declaration of graphs
  ...
  TLatex *lOptique = new TLatex(0,0.18, sOptique.c_str());
  lOptique->Draw();
...
int Find_bin_resolution(TH2F* h_ALL, float xmin, float xmax, int npeaks)
{
...
void Fit_YProjection_at_Resolution(TH1D* Output_resolution, int npeaks)
{
  double par[3000];
  int p;
