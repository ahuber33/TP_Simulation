...
TF1 *fit_gaus_resolution(TH1D* proj, TF1* fit_mean1, TF1* fit_mean2, TF1* fit_sigma1, TF1 *fit_sigma2, float flag_resolution)
{
  ...
  fit_gaus->FixParameter(0, fit_gaus1->GetParameter(0));
  fit_gaus->FixParameter(1, fit_gaus1->GetParameter(1));
  fit_gaus->FixParameter(2, fit_gaus1->GetParameter(2));
  fit_gaus->FixParameter(3, fit_gaus2->GetParameter(0));
  fit_gaus->FixParameter(4, fit_gaus2->GetParameter(1));
  fit_gaus->FixParameter(5, fit_gaus2->GetParameter(2));
...
void Analyse(const char* file_proton, const char* file_He1, const char* file_He2, const char* TreeName, const char* CanvasName, float pinhole, float B, float E, float Le1, float Le2, float D, float Lb1, float Lb2, float Sc_length, float Sc_thickness, float ZnS_thickness, bool Optique)
{
  ...
  TPad *pad4 = new TPad("pad4", "", 0., 0., 0.32, 0.49);
  TPad *pad5 = new TPad("pad5", "", 0.34, 0.25, 0.66, 0.49);
  TPad *pad6 = new TPad("pad6", "", 0.34, 0., 0.66, 0.26);
  TPad *pad7 = new TPad("pad7", "", 0.68, 0., 1, 0.49);
  TPad *pad8 = new TPad("pad8", "", 0.86, 0.51, 1, 1);
  pad1->Draw();
  pad1->SetLogz();
  pad2->Draw();
  pad3->Draw();
  ...
  if (Optique == true)
    {
      ...
      h_ALL = Histo_Parabole_Optique(Tree1, file_proton);
  ...
  TF1* fit_mean_proton = new TF1("fit_mean_proton", "pol2", -30, 0);
  TF1* fit_mean_He1 = new TF1("fit_mean_He1", "pol2", -15, 0);
  TF1* fit_mean_He2 = new TF1("fit_mean_He2", "pol2", -30, -3);
  ...
  float nsigma= 1.75;
  //Declaration of graphs
  TGraphErrors* MeanY_proton_norm = Fit_LargeurY_Parabole_NORM(h_proton, 1);
  TGraphErrors* MeanY_He1_norm = Fit_LargeurY_Parabole_NORM(h_He1, 1);
  TGraphErrors* MeanY_He2_norm = Fit_LargeurY_Parabole_NORM(h_He2, 1);
  TGraphErrors* Sigma_proton_norm = Fit_LargeurY_Parabole_NORM(h_proton, 2);
  TGraphErrors* Sigma_He1_norm = Fit_LargeurY_Parabole_NORM(h_He1, 2);
  TGraphErrors* Sigma_He2_norm = Fit_LargeurY_Parabole_NORM(h_He2, 2);
  TGraphErrors* Res_proton_He2 = Resolution(MeanY_proton_norm, MeanY_He2_norm, Sigma_proton_norm, Sigma_He2_norm, nsigma);
  TGraphErrors* Res_proton_He1 = Resolution(MeanY_proton_norm, MeanY_He1_norm, Sigma_proton_norm, Sigma_He1_norm, nsigma);
  TGraphErrors* Res_He1_He2 = Resolution(MeanY_He1_norm, MeanY_He2_norm, Sigma_He1_norm, Sigma_He2_norm, nsigma);

  //Draw & Color
  Res_proton_He2->Draw();
  Res_proton_He2->GetYaxis()->SetLimits(0,10);
  Res_proton_He2->GetYaxis()->SetRangeUser(0,10);
  Res_proton_He2->SetLineColor(kOrange);
  Res_proton_He1->Draw("same");
  Res_proton_He1->SetLineColor(kMagenta);
  Res_He1_He2->Draw("same");
  Res_He1_He2->SetLineColor(kCyan);
  float a = flag_resolution(Res_proton_He2);
  //  cout << "a = " << a << endl;
  TLine* l2 = new TLine(-16,1, -1, 1);
  l2->Draw("same");
  l2->SetLineColor(kRed);
  l2->SetLineWidth(2);
  TLine* l3 = new TLine(a, 0, a, 1);
  l3->Draw("same");
  ...
  //Title & Legend
  s = Form("x_{cut} =  %g mm", a);
  TLatex *t2 = new TLatex(a-2,-1, s.c_str());
  t2->SetTextColor(kRed+2);
  t2->SetTextFont(43);
  t2->SetTextSize(20);
  t2->Draw("same");
  Res_proton_He2->SetTitle("Evolution of resolution");
  //Res_proton_He2->GetXaxis()->SetTitle("X Position [mm]");
  Res_proton_He2->GetYaxis()->SetTitle("Resolution");
  auto legend2 = new TLegend(0.6,0.7,0.9,0.9);
  legend2->AddEntry(Res_proton_He2,"Proton - He2+","e");
  legend2->AddEntry(Res_proton_He1,"Proton - He1+","e");
  legend2->AddEntry(Res_He1_He2,"He1+ - He2+","e");
  legend2->Draw();
  ...
  //Draw & Color
  h_ALL->ProjectionY("proj", n, n, "");
  proj->Draw();
  proj->GetXaxis()->SetRangeUser(-1,-a);
  TF1* fit_gaus = fit_gaus_resolution(proj, fit_mean_proton, fit_mean_He2, fit_sigma_proton, fit_sigma_He2, a);
  //TF1* fit_gaus = fit_gaus_resolution_test(proj, MeanY_proton_norm, MeanY_He2_norm, Sigma_proton_norm, Sigma_He2_norm, a);
  fit_gaus->SetLineColor(kBlue);
  fit_gaus->Draw("same");

  //Title & Legend
  proj->SetTitle("Projection on x_{cut}");
  proj->GetXaxis()->SetTitle("Y Position [mm]");
  proj->GetYaxis()->SetTitle("N");

  //######################################################################
  //############################# PAD 6 ##################################
  //######################################################################
  pad6->cd();
  TH2F* Parabole_zoom = (TH2F*)h_ALL->Clone();
  ...
  //Title & Legend
  Parabole_zoom->SetTitle("Detector response zoom on x_{cut}");
  ...
  aze->ProjectionY("proj2", n, n, "");
  TF1* fit_gaus_Energie = new TF1("fit_gaus_Energie", "gaus", 0, 100000);
  proj2->Fit(fit_gaus_Energie, "QN");
  
  TLine* l6 = new TLine(a-0.05,0, a-0.05, fit_gaus_Energie->GetParameter(1));
  ...
  //Title & Legend
  s = Form("E_{cut} =  %d MeV", (int)fit_gaus_Energie->GetParameter(1)/1000);
  ...
  string sLe1 = Form("Le1 =  %g mm", Le1);
  TLatex *lLe1 = new TLatex(0,0.60, sLe1.c_str());
  lLe1->Draw();
  lLe1->SetTextSize(0.06);

  string sLe2 = Form("Le2 =  %g mm", Le2);
  TLatex *lLe2 = new TLatex(0,0.54, sLe2.c_str());
  lLe2->Draw();
  lLe2->SetTextSize(0.06);

  string sD = Form("D =  %g mm", D);
  TLatex *lD = new TLatex(0,0.48, sD.c_str());
  lD->Draw();
  lD->SetTextSize(0.06);

  string sLb1 = Form("Lb1 =  %g mm", Lb1);
  TLatex *lLb1 = new TLatex(0,0.42, sLb1.c_str());
  lLb1->Draw();
  lLb1->SetTextSize(0.06);

  string sLb2 = Form("Lb2 =  %g mm", Lb2);
  TLatex *lLb2 = new TLatex(0,0.36, sLb2.c_str());
  lLb2->Draw();
