...
void TPSimSteppingAction::UserSteppingAction(const G4Step *aStep){
  ...
  if(partname == "e-")
	  ...
	  ||(aStep->GetTrack()->GetCreatorProcess()->GetProcessName() == "phot"))
	{
	  if ((aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "GM_Gaz")
	    ...
	    {
	      // G4cout << "Déclenchement !!!" << G4endl;
	      // G4cout << "Processus de creation = " << aStep->GetTrack()->GetCreatorProcess()->GetProcessName() << G4endl;
	      // G4cout << "Step Length = " << aStep->GetTrack()->GetStepLength()/mm << G4endl;
	      //evtac->AddTrackLength(aStep->GetTrack()->GetStepLength()/mm);
	      // G4cout << "Track Length dans Gaz = " << evtac->GetTotalTrackLength() << G4endl;

	      // G4cout << "\nX = " << x << " mm " << G4endl;
  	      // G4cout << "Y = " << y << " mm " << G4endl;
  	      // G4cout << "Z = " << z << " mm " << G4endl;
  	      // G4cout << "R = " << r << " mm " << G4endl;
  	      // G4cout << "Rayon d'interaction = " << r << " mm " << G4endl;
  	      if(r > evtac->GetRmax()) {evtac->SetRmax(r);}
  	      if(r < evtac->GetRmin()) {evtac->SetRmin(r);}
  	      // G4cout << "Rmin = " << evtac->GetRmin() << G4endl;
  	      // G4cout << "Rmax = " << evtac->GetRmax() << G4endl;
  	      evtac->SetDeltaR(evtac->GetRmax() - evtac->GetRmin());
  	      // G4cout << "Delta R = " << evtac->GetDeltaR() << G4endl;
  ...
  if (Parent_ID ==0)
    {
      evtac->SetEstart(aStep->GetPreStepPoint()->GetKineticEnergy()/keV);
    }
